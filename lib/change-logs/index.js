...
class ChangeLogs {
    ...
    toFormResource(domain, persistence) {
        ...
        return Promise.map(
            ...
            (changeLog) => {
                ...
                if (ChangeLogClass) {
                    return BaseChangeLog.fromDocument(ChangeLogClass, changeLog).toFormResource(domain, persistence, cache);
                }
            },
            { concurrency: 1 }
        );
    ...
    static toFormResource(domain, persistence, instance) {
        return Promise.resolve()
            .then(() => (new ChangeLogs(instance)).toFormResource(domain, persistence))
        ;
    }
