...
class User {
    ...
    async getDisplayImage(persistence) {
        if (!this._displayImage) {
            debug(`getDisplayImage(): Need to find display image.`);
            const entity = await getEntity(this.domainName, ENTITY_NAME);
            const imagesRelationship = entity.getRelationshipByName('Images');
            const imageDocuments = await imagesRelationship.getDocuments(persistence, this.userDocument);
            debug(`getDisplayImage(): imageDocuments=`, imageDocuments);
        }
