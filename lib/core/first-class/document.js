...
class Document {
    ...
    toBasicResource(req) {
        ...
        if (this.displayImageUrl) {
            resource.link('displayImage', {
                title: "Display image",
                href: this.displayImageUrl
            });
        }
    ...
    toDocumentListResource(req) {
        // debug(`toDocumentListResource(): this.document=`, this.document);
        const resource = this.toBasicResource(req);
    ...
    toNotificationListResource(req) {
        const resource = this.toBasicResource(req);

        return resource;
    };

    async getDisplayImageUrl(persistence, req) {
        if (isUndefined(this.displayImageUrl)) {
            // debug(`getDisplayImageUrl(): need find...`);
            this.displayImageUrl = null;

            const entity = await getEntity(this.domainName, this.document.type);

            // preview image
            const imagesRelationship = entity.getRelationshipByName('Images');
            if (imagesRelationship) {
                const imageDocuments = await imagesRelationship.getDocuments(persistence, this.document);
                const previewImage = imageDocuments.find((image) => image.Type === 'PreviewImage');
                if (previewImage) {
                    this.displayImageUrl = previewImage.ImageURL;
                    return this.displayImageUrl;
                }
            }

            // No preview image defined, let's look in the overview.
            const overviewRelationship = entity.getRelationshipByName('Overview');
            if (overviewRelationship) {
                const overviewDocuments = await overviewRelationship.getDocuments(persistence, this.document);
                const firstOverview = overviewDocuments[0];
                if (firstOverview) {
                    const firstOverviewEntity = await getEntity(this.domainName, firstOverview.type);
                    const imagesRelationship = firstOverviewEntity.getRelationshipByName('Images');
                    const documents = await imagesRelationship.getDocuments(persistence, firstOverview);
                    const firstImage = documents[0];
                    // debug(`firstImage=`, firstImage);
                    if (firstImage) {
                        this.displayImageUrl = firstImage.ImageURL || null;
                    }
                }
            }
        }
