...
function extractTagValue(str, openTag, closeTag) {
    ...
    const pos1 = str.indexOf(openTag);
    const pos2 = str.indexOf(closeTag);
...
function getTokenSeq(str, openTag, closeTag) {
    ...
    while (str.includes(openTag)) {
        const bs = extractTagValue(str, openTag, closeTag);
        if (bs[0].length > 0) {
...
const matchAll = (text, regex) => {
    ...
    const matches = [];
    const statefulRE = new RegExp(regex);
    for (let m = statefulRE.exec(text); m !== null; m = statefulRE.exec(text)) {
        matches.push(m);
    }
    return matches;
};
