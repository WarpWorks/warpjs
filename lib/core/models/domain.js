    ...
    replaceOIDs() {
        this.getEntities().forEach((entity) => {
            ...
            entity.relationships.forEach((relationship) => {
                const target = this.findElementByID(relationship.getTargetEntity());
                if (target) {
                    relationship.setTargetEntity(target);
                } else {
                    throw new WarpWorksError(`Cannot find target entity for ${entity.name}.${relationship.name}`);
                }
            });
            ...
            entity.tableViews.forEach((tableView) => {
                tableView.tableItems.forEach((tableItem) => {
                    const target = this.findElementByID(tableItem.property);
                    if (target) {
                        tableItem.setProperty(target);
                    } else {
                        throw new WarpWorksError(`Cannot find property for ${entity.name}.${tableView.name}.${tableItem.name}.`);
                    }
                });
            });
            ...
            entity.pageViews.forEach((pageView) => {
                pageView.panels.forEach((panel) => {
                    panel.relationshipPanelItems.forEach((relationshipPanelItem) => {
                        const target = this.findElementByID(relationshipPanelItem.relationship);
                        if (target) {
                            relationshipPanelItem.setRelationship(target);
                        } else {
                            throw new WarpWorksError(`Cannot find relationship for ${entity.name}.${pageView.name}.${panel.name}.${relationshipPanelItem.name}.`);
                        }
                    });

                    panel.basicPropertyPanelItems.forEach((basicPropertyPanelItem) => {
                        const oid = basicPropertyPanelItem.basicProperty;
                        const target = this.findElementByID(oid);
                        if (target) {
                            basicPropertyPanelItem.setBasicProperty(target);
                        } else {
                            throw new WarpWorksError(`Cannot find basic property OID ${oid} for ${entity.name}.${pageView.name}.${panel.name}.${basicPropertyPanelItem.name}.`);
                        }
                    });
