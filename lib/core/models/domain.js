...
class Domain extends Base {
    ...
    getEntityByPersistenceId(persistenceId) {
        const entities = this.getEntities().filter((entity) => String(entity.persistenceId) === String(persistenceId));
        if (entities.length > 1) {
            throw new DomainError(`Multiple entities found with persistenceId='${persistenceId}'.`);
        } else if (entities.length) {
            return entities[0];
        } else {
            throw new DomainError(`Cannot find entity with persistenceId='${persistenceId}'.`);
        }
    }
