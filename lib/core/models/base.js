...
class Base {
    constructor(type, parent, id, name, desc) {
        ...
        */
        ...
        if (domain) {
            domain.idCounter = Math.max(domain.idCounter, id + 1);
        }
    }
    ...
    getInstancePath(persistence, instance) {
        return Promise.resolve()
            ...
            .then((ancestors) => ancestors.concat({
                ...
                displayType: this.name // FIXME: Use a label when it is added.
            }))
        ;
    }
    ...
    getDisplayName(instance) {
        ...
        if (name) {
            return name;
        } else if (instance.type === 'Paragraph' && instance.Heading) {
