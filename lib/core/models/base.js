...
class Base {
     ...
     */
    getParentData(persistence, instance) {
        if (this.hasParentInstance(instance)) {
            return Promise.resolve()
                .then(() => this.getDomain())
                .then((domain) => domain.getEntityById(instance.parentBaseClassID))
                .then((parentEntity) => Promise.resolve()
                    .then(() => parentEntity.getDocuments(persistence, { _id: instance.parentID }, true))
    ...
    getParent(persistence, instance) {
        return Promise.resolve()
            .then(() => this.getDomain().getParentEntityByParentBaseClassID(instance))
            .then((parentEntity) => {
                if (parentEntity) {
