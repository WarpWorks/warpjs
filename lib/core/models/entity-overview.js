...
// const debug = require('debug')('W2:models:entity-overview');
    ...
    return Promise.resolve()
        .then(() => {
            ...
            return Promise.reduce(
                ...
                (memo, relationship) => Promise.resolve()
                    .then(() => relationship.getDocuments(persistence, doc))
                    .then((targetDocs) => Promise.resolve()
                        .then(() => Promise.map(
                            targetDocs,
                            (targetDoc) => extractInfo(persistence, relationship.getTargetEntity(), recursiveCount - 1, targetDoc)
                        ))
                        .then((infos) => _.extend(memo, {
