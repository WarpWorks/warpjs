        ...
        return Promise.resolve()
            ...
            .then((resource) => Promise.resolve()
                ...
                .then((panelItems) => Promise.map(
                    panelItems,
                    (relationshipPanelItem) => relationshipPanelItem.toStudioResource(persistence, instance, docLevel, relativeToDocument)
                ))
                .then((items) => resource.embed('items', items))

                // panel actions
                .then(() => Promise.map(this.actions, (action) => action.toStudioResource(persistence, instance, docLevel, relativeToDocument)))
                .then((actions) => resource.embed('actions', actions))
