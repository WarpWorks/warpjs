...
class Panel extends Base {
    ...
    static fromFileJSON(jsonData, parent) {
        ...
        const newPanel = new Panel(parent, jsonData.id, jsonData.name, jsonData.desc);
        ...
        newPanel.separatorPanelItems = jsonData.separatorPanelItems.map(
            (json) => SeparatorPanelItem.fromFileJSON(json, newPanel)
        );

        newPanel.relationshipPanelItems = jsonData.relationshipPanelItems.map(
            (json) => RelationshipPanelItem.fromFileJSON(json, newPanel)
        );

        newPanel.basicPropertyPanelItems = jsonData.basicPropertyPanelItems.map(
            (json) => BasicPropertyPanelItem.fromFileJSON(json, newPanel)
        );

        newPanel.enumPanelItems = jsonData.enumPanelItems.map(
            (json) => EnumPanelItem.fromFileJSON(json, newPanel)
        );

        newPanel.actions = jsonData.actions.map(
            (json) => Action.fromFileJSON(json, newPanel)
        );
    ...
    static fromPersistenceJSON(persistence, json, parent) {
        return Promise.resolve(new Panel(parent, json.warpjsId, json.name, json.description))
