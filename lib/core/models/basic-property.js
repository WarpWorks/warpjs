...
class BasicProperty extends Base {
    ...
    static fromFileJSON(jsonData, parent) {
        ...
        const newProperty = new BasicProperty(parent, jsonData.id, jsonData.name, jsonData.desc, jsonData.propertyType);
    ...
    static fromPersistenceJSON(persistence, json, parent) {
        return Promise.resolve()
            .then(() => new BasicProperty(parent, json.warpjsId, json.name, json.description, json.propertyType))
            .then((instance) => Promise.resolve()
                .then(() => {
                    instance.defaultValue = json.defaultValue;
                    instance.constraints = json.constraints;
                    instance.examples = json.examples;
                })
                .then(() => instance)
            )
        ;
