...
class Enumeration extends Base {
    ...
    newInstance() {
        return (this.literals && this.literals.length && this.literals[0]) || "Undefined";
    }
    ...
    patch(updatePath, updatePathLevel, instance, updateValue) {
        const newValue = this.getAllElements().filter((lit) => lit.name === updateValue).pop();
        if (newValue) {
            instance[this.name] = updateValue;
        } else {
            throw new Error(`Invalid new value: ${updateValue}`);
        }
    }
}
