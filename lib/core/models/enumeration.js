...
class Enumeration extends Base {
    ...
    static fromFileJSON(jsonData, parent) {
        ...
        const newEnumeration = new Enumeration(parent, jsonData.id, jsonData.name, jsonData.desc);

        newEnumeration.literals = jsonData.literals.map(
            (literalJson) => Literal.fromFileJSON(literalJson, newEnumeration)
        );
    ...
    toPersistenceJSON() {
        return _.extend({}, super.toPersistenceJSON(), {
            ...
            embedded: [{
                parentRelnID: 50,
                parentRelnName: 'literals',
                entities: this.literals.map((literal) => literal.toPersistenceJSON())
            }]
        });
    }

    static fromPersistenceJSON(persistance, json, parent) {
        return Promise.resolve()
            .then(() => new Enumeration(parent, json.warpjsId, json.name, json.description))
