...
class Enumeration extends Base {
    ...
    toFormResource(persistence, instance) {
        return Promise.resolve()
            .then(() => Promise.map(this.literals, (literal) => literal.toFormResource(instance[this.name])))
            .then((literals) => {
                ...
                const {min, max} = converters.minMax(this.validEnumSelections);
