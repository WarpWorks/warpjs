...
class Relationship extends Base {
    ...
    toFormResource(persistence, instance, docLevel, relativeToDocument) {
        ...
        const resource = warpjsUtils.createResource(resourceUrl, _.extend({}, this.toFormResourceBase(), {
    ...
    toStudioResource(persistence, instance, docLevel, relativeToDocument) {
        ...
        return Promise.resolve()
            .then(() => RoutesInfo.expand(studioConstants.routes.relationship, {
                domain,
                type: instance.type, // TODO: Find stable type.
                id: instance.id,
                relationship: this.name
            }))
            ...
            .then((resource) => Promise.resolve()
                ...
                .then((docs) => {
                    ...
                    // FIXME: Convert this into embedded with all pages links.
                    if (resource.totalSize > resource.pageEnd) {
                        resource.link('nextPage', RoutesInfo.expand(studioConstants.routes.relationshipPage, {
                            domain,
                            type: instance.type,
                            id: instance.id,
                            relationship: this.name,
                    ...
                    return this.getTargetEntity().toStudioResource(persistence, instance, docLevel, relativeToDocument, docs);
                })
                .then((model) => {
                    resource.model = model;
                })
