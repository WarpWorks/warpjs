...
class Relationship extends Base {
    ...
    getDocuments(persistence, instance) {
        ...
        return Promise.resolve()
            ...
            .then((documents) => documents.filter((doc) => doc.type))
    ...
    patch(updatePath, updatePathLevel, instance, updateValue, patchAction) {
        ...
        const targetEntity = this.getTargetEntity();
        // debug(`patch(): name=${this.name}; targetEntity=${targetEntity.name} (${targetEntity.entityType})`);
    ...
    clone(persistence, instance, clone) {
        if (this.isAggregation) {
            return Promise.each(
                this.getDocuments(persistence, instance),
                (document) => Promise.resolve()
                    .then(() => this.getDomain().getEntityByInstance(document))
                    .then((targetEntity) => Promise.resolve()
                        .then(() => targetEntity.clone(persistence, document, clone.id))
                        .then((clonedDocument) => Promise.resolve()
                            .then(() => {
                                if (!targetEntity.isDocument()) {
            ...
            return Promise.resolve()
                .then(() => Promise.each(
                    ...
                    (targetReference) => Promise.resolve()
                        .then(() => _.omit(targetReference, ['_id']))
                        .then((newRef) => _.extend(newRef, {id: targetReference._id}))
                        .then((newRef) => this.addAssociation(clone, newRef))
                ))
            ;
        }
    }
