...
class Relationship extends Base {
    ...
    async addAssociation(instance, data, persistence, isReverse) {
        ...
        // Add the bi-directional association. Only applies if target entity's
        // entityType is Document.
        const targetEntity = this.getTargetEntity();
        if (targetEntity.isDocument()) {
            if (!persistence) {
            ...
            const domainEntity = this.getDomain();
            const targetRealEntity = domainEntity.getEntityByInstance(data);
            const targetDocument = await targetRealEntity.getInstance(persistence, data.id);
            ...
            await targetRealEntity.updateDocument(persistence, targetDocument);
        }
