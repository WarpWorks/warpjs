    ...
    toStudioResource(persistence, instance, docLevel, relativeToDocument, routes, style) {
        ...
        return Promise.resolve()
            ...
            .then((resource) => Promise.resolve()
                .then(() => RoutesInfo.expand(routes.instance, {
                    domain,
                    type: this.getTargetEntity().name,
                    profile: 'withChildren'
                }))
                .then((href) => resource.link('types', href))

                .then(() => {
                    if (this.name === 'parentClass') {
                        // FIXME: This is a hack for parentClass.
                        return this.getTargetEntity().getDocuments(persistence, { warpjsId: instance.parentClass });
                    } else if (this.name === 'targetEntity') {
                        // FIXME: This is a hack for targetEntity.
                        return this.getTargetEntity().getDocuments(persistence, { warpjsId: instance.targetEntity });
                    }
