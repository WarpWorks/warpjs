...
class Relationship extends Base {
    ...
    addEmbedded(instance) {
        if (!instance.embedded) {
            instance.embedded = [];
        }

        let filteredEmbedded = instance.embedded.filter((embedded) => embedded.parentRelnID === this.id);
        if (!filteredEmbedded.length) {
            ...
            filteredEmbedded = instance.embedded.filter((embedded) => embedded.parentRelnID === this.id);
        }
        ...
        embedded.entities.push(this.getTargetEntity().newInstance());
