...
class Relationship extends Base {
    ...
    toFormResource() {
        ...
        const targetEntity = this.targetEntity;

        if (targetEntity && targetEntity.length) {
            const target = targetEntity[0];

            const targetUrl = RoutesInfo.expand('W2:content:schema-type', {
                domain: target.getDomain().name,
                type: target.name
            });

            const targetResource = warpjsUtils.createResource(targetUrl, {
                type: target.name,
                name: target.name,
                desc: target.desc
            });

            resource.embed('targetEntities', targetResource);
        }
