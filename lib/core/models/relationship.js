...
class Relationship extends Base {
                ...
                return Promise.resolve()
                    .then(() => {
                        if (instance.embedded) {
                            const embeddedDocs = instance.embedded.filter((embedded) => embedded.parentRelnID === this.id);
    ...
    addEmbedded(instanceEmbedded, docLevel, level) {
        console.log("relationship.addEmbedded(): instanceEmbedded=", instanceEmbedded, "; docLevel=", docLevel, "; level=", level);
        ...
        if (isLastDocLevel(docLevel, level)) {
            embedded.entities.push(targetEntity.newInstance());
        } else {
            ...
            targetEntity.addEmbedded(entityInstance, docLevel, level + 2);
        }
    }
