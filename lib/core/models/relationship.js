...
class Relationship extends Base {
    ...
    toStudioResource(persistence, instance, docLevel, relativeToDocument, routes, style) {
        // if (this.getTargetEntity().isDocument()) {
        docLevel = docLevel.empty().addRelationship(this.name);
        // } else {
        //     docLevel = docLevel.addRelationship(this.name);
        // }
        // debug(`relativeToDocument=`, relativeToDocument);
        // debug(`    new: domain:${relativeToDocument.domain}; type:${instance.type}; id:${instance.id}`);
        ...
        // debug(`toStudioResource(): name=${this.name}; target.isdocument? ${this.getTargetEntity().isDocument()}; instance.id=${instance.id}`);
        // debug(`toStudioResource(): relativeToDocument=`, relativeToDocument);
        // debug(`toStudioResource(): this=`, this);
        // debug(`toStudioResource(): instance=`, instance);

        // if (this.name === 'targetEntity') {
        //     debug(`toStudioResource():     is targetEntity...`);
        //     relativeToDocument = {
        //         domain: relativeToDocument.domain,
        //         type: TYPE,
        //         id: instance.id,
        ...
        return Promise.resolve()
            // .then(() => debug(`Relationship.toStudioResource(): name=${this.name} instance=`, instance))
            .then(() => RoutesInfo.expand(routes.relationship, {
                domain: relativeToDocument.domain,
                type: relativeToDocument.type,
                id: relativeToDocument.id,
                relationship: this.name
            }))
            .then((href) => warpjsUtils.createResource(href, _.extend({}, this.toFormResourceBase(), {
                docLevel: docLevel.toString()
            })))
            .then((resource) => Promise.resolve()
                // .then(() => debug(`relationship: name=${this.name}, getTargetEntity()=`, this.getTargetEntity()))
                .then(() => {
                    ...
                    if (this.name === 'parentClass') {
                        ...
                        // This is the target relationship of a panel item (in a
                        // PageView)
                        // debug(`This is a RelationshipPanelItem.relationship`);
                        // debug(`instance.relationship=${instance.relationship}`);
                        ...
                        const entitiesUrl = RoutesInfo.expand(routes.relationship, {
                            ...
                            id: relativeToDocument.id,
                            relationship: this.name,
                            profile: 'types' // FIXME: Use a constant.
                        });
                        resource.link('types', entitiesUrl);
