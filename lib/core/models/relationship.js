...
const WarpjsModelError = require('./error');
...
class Relationship extends Base {
    ...
    updateDesc() {
        const target = this.hasTargetEntity() && typeof this.getTargetEntity() === "object"
    ...
    deleteAllInstances(persistence, instance) {
        const targetEntity = this.getTargetEntity();
    ...
    async recursiveGetDocuments(persistence, document, cumulator = []) {
        const documents = await this.getDocuments(persistence, document);
        return Promise.reduce(
            documents,
            async (cumulator, relatedDocument) => {
                cumulator.push(relatedDocument);
                return this.recursiveGetDocuments(persistence, relatedDocument, cumulator);
            },
            cumulator
        );
    }
    ...
    async getDocuments(persistence, instance) {
        ...
        if (this.isAggregation) {
            if (!targetEntity.isDocument()) {
                return references.sort(warpjsUtils.byPositionThenName);
            }

            const children = await targetEntity.getChildren(persistence, instance.id);
            children.sort(warpjsUtils.byPositionThenName);
            return children;
            ...
            documents.sort(warpjsUtils.byPositionThenName);
    ...
    async addAggregation(persistence, entity, document, childEntityName) {
        const documents = await this.getDocuments(persistence, document);
        const templateDocument = documents.find((doc) => doc.Name === 'TEMPLATE');

        let child;
        if (templateDocument) {
            const templateEntity = this.getDomain().getEntityByInstance(templateDocument);
            child = await templateEntity.clone(persistence, templateDocument, null, document.Version || DEFAULT_VERSION);
        } else {
            const defaultTargetEntity = this.getTargetEntity();

            let desiredTargetEntity = defaultTargetEntity;

            if (childEntityName && childEntityName !== defaultTargetEntity.name) {
                const requestedTargetEntity = this.getDomain().getEntityByName(childEntityName);
                if (requestedTargetEntity) {
                    // Checking if this is a child of the current entity.
                    const childEntities = defaultTargetEntity.getChildEntities(true, true);
                    const foundRequestedTargetEntity = Boolean(childEntities.find((childEntity) => childEntity.name === childEntityName));
                    if (!foundRequestedTargetEntity) {
                        throw new Error(`Entity '${childEntityName}' is not a child of target entity '${defaultTargetEntity.name}'.`);
                    }

                    desiredTargetEntity = requestedTargetEntity;
                } else {
                    throw new Error(`Invalid entity '${childEntityName}'.`);
                }
            }

            child = await desiredTargetEntity.createContentChildForRelationship(this, entity, document);
        }

        child.Version = document.Version || DEFAULT_VERSION;
     ...
     *      }
     */
    async addAssociation(instance, data, persistence, isReverse) {
        const targetEntity = this.getDomain().getEntityByInstance(data);
        if (data && data.type && !data.typeID) {
            data = extend({}, data, {
                typeID: targetEntity.id
            });
        }
        ...
        const filteredData = association.data.find((reference) => {
            // FIXME: Use typeID to allow entity name change.
            return (reference._id === data.id && reference.type === data.type);
        });

        if (filteredData) {
            // already existing, we don't need to add the reverse because it
            // should be there too.
            filteredData.typeID = data.typeID || filteredData.typeID;
            filteredData.desc = data.desc;
        ...
        if (isReverse) {
            // This is already the reverse document, so don't try to create the
            // reverse one.
            return instance;
        }

        // Add the bi-directional association.
        if (targetEntity.isDocument()) {
            if (!persistence) {
                throw new WarpjsModelError(`Missing 'persistence' param.`);
            }

            let targetDocument = await targetEntity.getInstance(persistence, data.id);
            if (targetDocument.type === instance.type && targetDocument.id === instance.id) {
                targetDocument = instance;
            }

            // The reverse relationship is only defined on document entity.
            const reverseRelationship = this.getReverseRelationship();
            if (reverseRelationship) {
                const reverseData = Object.freeze({
                    id: instance.id,
                    type: instance.type,
                    typeID: instance.typeID,
                    desc: 'Bi-directional auto-created'
                });

                await reverseRelationship.addAssociation(targetDocument, reverseData, persistence, true);
            }
            await targetEntity.updateDocument(persistence, targetDocument);
        }
     ...
     */
    async removeAssociation(instance, data, persistence, isReverse) {
        const targetEntity = this.getDomain().getEntityByInstance(data);
        ...
        if (targetEntity.isDocument()) {
            ...
            const targetDocument = await targetEntity.getInstance(persistence, data.id);
            const reverseRelationship = this.getReverseRelationship();
            const reverseData = Object.freeze({
                id: instance.id,
                type: instance.type
            });
            await reverseRelationship.removeAssociation(targetDocument, reverseData, persistence, true);
            await targetEntity.updateDocument(persistence, targetDocument);
        }
    ...
    async clone(persistence, instance, clone, version) {
        if (this.isAggregation) {
            const docs = await this.getDocuments(persistence, instance);
            ...
            await Promise.each(
                ...
                async (document) => {
                    ...
                    const clonedDocument = await targetEntity.clone(persistence, document, clone.id, version);
            ...
            await Promise.each(
                ...
                async (targetReference) => {
                    ...
                    await this.addAssociation(clone, newRef, persistence);
                }
            );
        }
    }
    ...
    isReverse() {
        return this.isReverseName(this.name);
    }
    ...
    get reverseName() {
        ...
        // name.
        return this.generateReverseName(this.id);
    }
    ...
    getReverseRelationship() {
        if (this.isReverse()) {
            const m = this.name.match(REVERSE_RELATIONSHIP_NAME_RE);
            if (m) {
                const relationshipId = parseInt(m[1], 10);
                return this.getDomain().getElementById(relationshipId);
            } else {
                throw new WarpjsModelError(`Invalid reverse relationship name '$${this.name}'.`);
            }
        } else {
            return this.getTargetEntity().getRelationshipByName(this.reverseName);
        }
    }
}
