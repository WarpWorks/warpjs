...
class Entity extends Base {
    ...
    async updateDocument(persistence, instance, skipNotifications) {
        instance.lastUpdated = (new Date()).toISOString();
    ...
    async clone(persistence, instance, parentID) {
        ...
        await Promise.each(
            ...
            async (relationship) => relationship.clone(persistence, instance, clone)
        );
    ...
    async notifyUsers(persistence, instance) {
        ...
        const userEntity = this.getDomain().getEntityByName('User');

        const authorsRelationship = this.getRelationshipByName('Authors');
        users = await findUsersToNotify(persistence, instance, authorsRelationship, users);

        const contributorsRelationship = this.getRelationshipByName('Contributors');
        users = await findUsersToNotify(persistence, instance, contributorsRelationship, users);
        ...
        users = await findUsersToNotify(persistence, instance, followsReverseRelationship, users);

        await Promise.each(users, async (user) => Notification.addToUser(persistence, userEntity, user, instance));
    }
}

const findUsersToNotify = async (persistence, instance, relationship, users) => {
    const newUsers = relationship
        ? await relationship.getDocuments(persistence, instance)
        : []
    ;

    return newUsers.reduce(
        (memo, newUser) => {
            const duplicate = memo.find((user) => user.id === newUser.id);
            if (!duplicate) {
                memo.push(newUser);
            }
            return memo;
