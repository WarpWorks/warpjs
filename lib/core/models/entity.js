...
class Entity extends Base {
    ...
    setRootEntityStatus(declareAsRootEntity) {
        if (this.isRootInstance) {
            ...
            const relName = this.namePlural.charAt(0).toUpperCase() + this.namePlural.slice(1);
            const rel = this.getDomain().getRootInstance().addNewRelationship(this, true, relName);
            rel.targetMax = '*';
    ...
    async getOverviews(persistence, instance) {
        ...
        return overviews.map((overview) => pick(overview, [ 'Heading', 'Content' ]));
    }
    ...
    canBeInstantiated() {
        ...
        const parentAggs = this.getAllParentAggregations();
    ...
    getBasicProperties(ignoreInheritedProperties) {
        if (!ignoreInheritedProperties && this.hasParentClass()) {
            ...
            return clone(this.basicProperties);
        }
    }
    ...
    getDefaultPageView() {
        for (let idx = 0; idx < this.pageViews.length; idx++) {
    ...
    getDefaultTableView() {
        for (let idx = 0; idx < this.tableViews.length; idx++) {
    ...
    getAllDerivedEntities() {
        ...
        for (const i in domain.entities) {
            const entity = domain.entities[i];
            if (entity.hasParentClass()) {
                const parent = entity.getParentClass();
    ...
    getAllParentAggregations() {
        ...
        for (const i in entities) {
            ...
            const aggRels = entity.getAggregations();
            for (const k in aggRels) {
                const rel = aggRels[k];
    ...
    addNewBasicProperty(name, desc, propertyType) {
        ...
        const newBasicProperty = new BasicProperty(this, id, name, desc, propertyType);
    ...
    addNewEnum(name, desc) {
        const id = this.getDomain().createNewID();
        const newEnum = new Enumeration(this, id, name, desc);
    ...
    addNewRelationship(target, isAggregation, name) {
        ...
        const newRelationship = new Relationship(this, target, id, isAggregation, name);
    ...
    addNewPageView(name, desc) {
        const id = this.getDomain().createNewID();
        const newPageView = new views.PageView(this, id, name, desc);
    ...
    addNewTableView(name, desc) {
        const id = this.getDomain().createNewID();
        const newTableView = new views.TableView(this, id, name, desc);
        ...
        if (this.isDocument()) {
            ...
            resource.data = await Promise.map(docs,
                async (doc, indexPosition) => {
                    ...
                    return extend({}, subData, { indexPosition });
                }
            );
    ...
    async getChildren(persistence, parentData) {
        ...
        // relnPosition and relnDesc gets saved to DB when we update a document
        // that was extracted from a relationship.
        const noReln = docs.map((d) => omit(d, [ 'relnPosition', 'relnDesc' ]));
        return noReln.sort(warpjsUtils.byPositionThenName);
    }

    async clone(persistence, instance, parentID, version) {
        let documentClone = pick(instance, [
            'parentID',
            'parentRelnID',
            'parentRelnName',
            'parentBaseClassID',
        ...
        if (this.isDocument() && parentID) {
            documentClone.parentID = parentID;
        }

        // Basic Properties
        await Promise.each(
            this.getBasicProperties(),
            async (basicProperty) => basicProperty.clone(persistence, instance, documentClone)
        );

        // Replace with specific version if defined.
        documentClone.Version = version || documentClone.Version || DEFAULT_VERSION;

        // Enumerations
        await Promise.each(
            this.getEnums(),
            async (enumeration) => enumeration.clone(persistence, instance, documentClone)
        );

        if (this.isDocument()) {
            documentClone = await this.createDocument(persistence, documentClone);
        } else {
            documentClone._id = uuid();
        }

        // Relationships
        await Promise.each(
            this.getRelationships(),
            async (relationship) => relationship.clone(persistence, instance, documentClone, version)
        );
        ...
        if (this.isDocument() && version) {
            ...
            if (!parentID) {
                ...
                nameBasicProperty.setValue(documentClone, currentName);
            }
            ...
            if (versionBasicProperty) {
                versionBasicProperty.setValue(documentClone, version);
            }
            ...
            if (statusEnum) {
                ...
                if (!parentID || (currentStatus !== DOCUMENT_STATUS.INHERITANCE)) {
                    statusEnum.setValue(documentClone, DOCUMENT_STATUS.NEW_VERSION_STATUS);
                }
            }
            ...
            if (predecessorRelationship && instance.Name !== 'TEMPLATE') {
                // The precessor association should be cleaned.
                const previousPredecessor = predecessorRelationship.getTargetReferences(documentClone);
                ...
                await predecessorRelationship.addAssociation(documentClone, data, persistence);
            }
        }
        // Replace the Name and Position only for the top document.
        if ((this.isDocument() && !parentID && !version) || (documentClone.Name === 'TEMPLATE')) {
            documentClone.Name = 'ChangeThisName';
            documentClone.Position = '';
        }

        if (this.isDocument()) {
            await this.updateDocument(persistence, documentClone);
        }
