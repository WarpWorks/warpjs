...
class Entity extends Base {
    ...
    getOverview(persistence, instance) {
        console.warn(```*** DEPRECATION *** Do not use getOverview().```);
    ...
    getOverviews(persistence, instance) {
        console.warn(```*** DEPRECATION *** Do not use getOverviews().```);
        return Promise.resolve()
    ...
    createDocument(persistence, instance) {
        ...
        return Promise.resolve()
            .then(() => persistence.save(this.getBaseClass().name, instance))
        ;
    }

    removeDocument(persistence, instance) {
        return Promise.resolve()
            .then(() => persistence.remove(this.getBaseClass().name, instance))
        ;
    }
    ...
    updateDocument(persistence, instance) {
        instance.lastUpdated = (new Date()).toISOString();
        return Promise.resolve()
            .then(() => persistence.update(this.getBaseClass().name, instance))
        ;
    }
    ...
    patch(updatePath, updatePathLevel, instance, updateValue, patchAction) {
        ...
        if (currentPatch[0] === 'Basic') {
            ...
            return Promise.resolve()
                .then(() => this.getBasicPropertyByName(currentPatch[1]))
            ...
            return Promise.resolve()
                .then(() => this.getEnumByName(currentPatch[1]))
                .then((model) => model.patch(updatePath, updatePathLevel + 1, instance, updateValue))
            ;
        } else {
    ...
    getChildren(persistence, parentData) {
        ...
        return Promise.resolve()
            .then(() => persistence.documents(this.getBaseClass().name, parentData, true))
        ;
    }

    clone(persistence, instance, parentID) {
        return Promise.resolve()
            .then(() => _.pick(instance, [
                'parentID',
                'parentRelnID',
                'parentRelnName',
                'parentBaseClassID',
            ...
            .then((clone) => Promise.resolve()
                ...
                .then(() => Promise.each(
                    ...
                    (basicProperty) => basicProperty.clone(persistence, instance, clone)
                ))
                ...
                .then(() => Promise.each(
                    this.getEnums(),
                    (enumeration) => enumeration.clone(persistence, instance, clone)
                ))

                .then(() => {
                    if (this.isDocument()) {
                        return this.createDocument(persistence, clone);
                    } else {
                        clone._id = uuid();
                        return clone;
                    }
            ...
            .then((clone) => Promise.resolve()
                ...
                .then(() => Promise.each(
                    ...
                    (relationship) => Promise.resolve()
                        .then(() => relationship.clone(persistence, instance, clone))
                ))

                .then(() => {
                    if (this.isDocument()) {
                        return this.updateDocument(persistence, clone);
                    }
                })
