...
class Entity extends Base {
    ...
    toResource(instance, lastDocumentLevelHref, domain) {
    ...
    toStudioResource(persistence, instance, docLevel, relativeToDocument, routes, docs) {
        const domain = relativeToDocument.domain;

        if (this.isDocument()) {
            const href = RoutesInfo.expand(routes.entities, {
                domain,
                type: this.name
            });
            const resource = warpjsUtils.createResource(href, _.extend({}, this.toFormResourceBase(), {
            ...
            const entities = existingDocs.map((doc) => {
                ...
                const docResource = this.toResource(doc, relativeToDocument.href, relativeToDocument.domain);
            ...
            return Promise.resolve()
                ...
                .then((resource) => Promise.resolve()
                    ...
                    .then((pageView) => Promise.map(
                        docs,
                        (doc, indexPosition) => Promise.resolve()
                            .then(() => pageView.toStudioResource(persistence, doc, docLevel.concat(`Entity:${doc._id}`), relativeToDocument, routes))
                            .then((data) => _.extend({}, data, { indexPosition }))
                    ))
                    .then((data) => {
