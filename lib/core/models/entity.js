...
const studioConstants = require('./../../../server/studio/constants');
...
class Entity extends Base {
    ...
    toStudioResource(persistence, instance, docLevel, relativeToDocument, docs) {
        const domain = this.getDomain().name;

        if (this.isDocument()) {
            const href = RoutesInfo.expand(studioConstants.routes.entities, {
                domain,
                type: this.name
            });
            const resource = warpjsUtils.createResource(href, _.extend({}, this.toFormResourceBase(), {
            ...
            const data = existingDocs.map((doc) => [
                RoutesInfo.expand(studioConstants.routes.instance, {
                    domain,
                    type: doc.type,
                    id: doc.id
                })
            ...
            const entities = existingDocs.map((doc) => {
                ...
                const docResource = this.toResource(doc, relativeToDocument.href);
                docResource.docLevel = docLevel.concat(`Entity:${doc.id}`).join('.');
                return docResource;
            });
            resource.embed('entities', entities);
            ...
            return Promise.resolve()
                .then(() => warpjsUtils.createResource(relativeToDocument.href, this.baseJSON()))
                .then((resource) => Promise.resolve()
                    .then(() => this.getPageView(config.views.content))
                    .then((pageView) => Promise.map(
                        docs,
                        (doc, indexPosition) => Promise.resolve()
                            .then(() => pageView.toStudioResource(persistence, doc, docLevel.concat(`Entity:${doc._id}`), relativeToDocument))
                            .then((data) => _.extend({}, data, { indexPosition }))
                    ))
                    .then((data) => {
    ...
    save(persistence, domainPersistenceId) {
        return Promise.resolve()
            ...
            .then((entityJson) => {
                ...
                return Promise.resolve()
                    .then(() => persistence.save(TYPE, entityJson))
    ...
    toPersistenceJSON() {
        ...
        // Child elements:
        json.embedded.push(this.mapChildrenPersistenceJSON(17, 'basicProperties', this.getBasicProperties(true)));
        json.embedded.push(this.mapChildrenPersistenceJSON(21, 'enums', this.getEnums(true)));
        json.embedded.push(this.mapChildrenPersistenceJSON(18, 'relationships', this.getRelationships(true)));
    ...
    fromPersistenceJSON(persistence, json) {
        return Promise.resolve()
            ...
            // Basic properties
            .then(() => this.fromPersistenceEmbeddedJson(persistence, json.embedded, 17, BasicProperty, 'basicProperties'))

            // Enumerations
            .then(() => this.fromPersistenceEmbeddedJson(persistence, json.embedded, 21, Enumeration, 'enums'))

            // Relationships
            .then(() => this.fromPersistenceEmbeddedJson(persistence, json.embedded, 18, Relationship, 'relationships'))
