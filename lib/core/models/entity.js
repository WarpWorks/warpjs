...
class Entity extends Base {
    ...
    canBeEditedBy(persistence, instance, user) {
        const plugin = plugins.getAuthPlugin(config);

        if (!plugin) {
            // If there are no auth-module, assume everyone have write access.
            return Promise.resolve(true);
        } else {
            const adminRoleName = plugin.config.roles.admin;
            const userAdminRole = user.Roles.filter((userRole) => userRole.Name === adminRoleName).pop();
            if (userAdminRole) {
                // Assume admin users always can write every where.
                return Promise.resolve(true);
            }
        }
            ...
            .then(() => Promise.reduce(
                ...
                (canWrite, association) => {
                    ...
                    return Promise.resolve()
                        ...
                        .then((roles) => authorization.hasAnyRoles(user.Roles, roles))
                    ;
                },
                false
            ))
    ...
    getRelationshipById(id) {
        ...
        return (relationships.length) ? relationships.pop() : null;
    }
