...
class Entity extends Base {
    ...
    toFormResource(persistence, instance, docLevel, docs, lastDocumentLevelHref) {
        const domain = this.getDomain().name;
        ...
        if (this.isDocument()) {
            ...
            const entities = existingDocs.map((doc) => {
                const resource = this.toResource(doc, lastDocumentLevelHref);
            ...
            const resource = warpjsUtils.createResource(lastDocumentLevelHref, this.baseJSON());
            const pv = this.getPageView(config.views.content);

            return Promise.map(docs,
                (doc, indexPosition) => Promise.resolve()
                    .then(() => pv.toFormResource(persistence, doc, docLevel.concat(`Entity:${doc._id}`), lastDocumentLevelHref))
