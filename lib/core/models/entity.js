...
class Entity extends Base {
    ...
    createContentChildForRelationship(relationship, parentEntity, parentInstance) {
        ...
        this.getBasicProperties().forEach((prop) => _.extend(newInstance, {
            [prop.name]: prop.newInstance()
        }));

        this.getEnums().forEach((prop) => _.extend(newInstance, {
            [prop.name]: prop.newInstance()
        }));

        this.getRelationships().forEach((reln) => reln.newInstance(newInstance));

        // Just so it becomes obvious to the user.
        newInstance.Name = 'ChangeThisName';
