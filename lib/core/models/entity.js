...
class Entity extends Base {
    ...
    toFormResource(persistence, instance, docLevel, docs) {
        ...
        if (this.entityType === 'Embedded') {
            ...
            return Promise.map(docs, (doc, indexPosition) => {
                return Promise.resolve(pv.toFormResource(persistence, doc, docLevel.concat(`Position:${indexPosition}`)))
                    .then((data) => {
    ...
    patcher(updatePath, updatePathLevel, updateValue) {
        const currentPatch = updatePathInfo(updatePath, updatePathLevel);
        return Promise.resolve()
            .then(() => {
                if (currentPatch[0] === 'Basic') {
                    return this.getBasicProperties().filter((bp) => bp.name === currentPatch[1]).pop();
                }
            })
            .then((model) => {
                if (model) {
                    return model.patcher(updatePath, updatePathLevel + 1, updateValue);
                }
            })
        ;
    }
