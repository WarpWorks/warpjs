...
const Documents = require('./../first-class/documents');
...
class Entity extends Base {
    ...
    async toFormResource(persistence, instance, docLevel, docs, relativeToDocument) {
        ...
        const domainFull = this.getDomain();
        ...
        if (this.isDocument()) {
            ...
            const bestDocuments = await Documents.bestDocuments(persistence, domainFull, existingDocs);

            const data = bestDocuments.map((doc) => [
            ...
            resource.data = data;
            const entities = bestDocuments.map((doc) => {
