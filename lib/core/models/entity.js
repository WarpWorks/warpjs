...
class Entity extends Base {
    ...
    createDocument(persistence, instance) {
        return persistence.save(this.getBaseClass().name, JSON.parse(JSON.stringify(instance)));
    }

    removeDocument(persistence, instance) {
        return persistence.remove(this.getBaseClass().name, JSON.parse(JSON.stringify(instance)));
    }

    updateDocument(persistence, instance) {
        return persistence.update(this.getBaseClass().name, JSON.parse(JSON.stringify(instance)));
    }
    ...
    toResource(instance) {
        ...
        if (instance) {
            ...
            this.getBasicProperties().forEach((basicProperty) => {
                resource[basicProperty.name] = instance[basicProperty.name];
            });
        } else {
    ...
    toFormResource(persistence, instance, docLevel, docs) {
        const domain = this.getDomain().name;

        const resource = warpjsUtils.createResource(
            RoutesInfo.expand('W2:content:entities', {
                domain,
                type: this.name
            }),
            this.baseJSON()
        ...
        if (this.entityType === 'Embedded') {
            ...
            return Promise.map(docs, (doc, indexPosition) => {
                return Promise.resolve(pv.toFormResource(persistence, doc, docLevel.concat(`Entity:${doc._id}`)))
                    .then((data) => {
            ...
            const basicProperties = this.getBasicProperties().map((basicProperty) => basicProperty.name);
    ...
    patch(updatePath, updatePathLevel, instance, updateValue) {
        ...
        if (currentPatch[0] === 'Basic') {
            ...
            return Promise.resolve()
                .then(() => this.getBasicProperties().filter((bp) => bp.name === currentPatch[1]).pop())
                .then((model) => model.patch(updatePath, updatePathLevel + 1, instance, updateValue))
            ;
        } else if (currentPatch[0] === 'Relationship') {
            return Promise.resolve()
                .then(() => this.getRelationships())
                .then((relationships) => relationships.filter((relationship) => relationship.name === currentPatch[1]))
                .then((relationships) => relationships[0])
                .then((relationship) => relationship.patch(updatePath, updatePathLevel + 1, instance, updateValue))
            ;
        } else if (currentPatch[0] === 'Enum') {
            return Promise.resolve()
                .then(() => this.getEnums().filter((en) => en.name === currentPatch[1]).pop())
                .then((model) => model.patch(updatePath, updatePathLevel + 1, instance, updateValue))
            ;
        } else {
    ...
    addEmbedded(instance, docLevel, level) {
        console.log("entity.addEmbedded(): instance=", instance, "; docLevel=", docLevel, "; level=", level);
        ...
        relationshipEntity.addEmbedded(instance.embedded, docLevel, level);
