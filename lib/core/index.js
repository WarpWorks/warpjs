...
class WarpWorks {
    ...
    getPersistence(dbName) {
        ...
        return new Persistence(config.persistence.host, dbName || constants.CORE.DOMAIN);
    }
    ...
    getCoreDomain() {
        if (!this.coreDomain) {
            const content = this.readFile(constants.CORE.FILE);
    ...
    parseSMN(smn) {
        ...
        for (idx in smnFileMerged) {
            ...
            while (body.includes("{")) {
                ...
                const s = utils.extractTagValue(body, "{", "}");
                token = s[1];
                body = s[0] + (s.length > 2 ? s[2] : "");
                const aggregations = token.split(",");
                for (const j in aggregations) {
                    let agg = aggregations[j].split(":");
                    if (agg.length === 1) {
                        let sr = agg[0].replace("*", "");
                        sr = sr.replace("+", "");
                        sr += "s";
