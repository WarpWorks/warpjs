...
class WarpWorks {
    ...
    getDomainByName(persistence, domainName) {
        if (persistence && !domainName) {
            // Compatible with new API.
            domainName = persistence;
            persistence = undefined;
        }
        return new Promise((resolve, reject) => {
            const domainInfo = this.domains.filter((domainInfo) => domainInfo.schema.name === domainName).pop();

            const domainFilePath = path.resolve(this.getDir('domains', `${domainName}.jsn`));
            fs.stat(domainFilePath, (err, jsnStat) => {
                if (err) {
                    reject(err);
                } else if (!domainInfo || (domainInfo.mtime !== jsnStat.mtime.getTime())) {
                    ...
                    this.domains.push({
                        schema: domain,
                        mtime: jsnStat.mtime.getTime()
                    });
                    resolve(domain);
                } else {
                    resolve(domainInfo.schema);
                }
            });
        });
    }
    ...
    expireDomainCache(domainName) {
        this.domains = this.domains.filter((domainInfo) => domainInfo.schema.name !== domainName);
    }
    ...
    createInstanceFromJSON(jsonData, type, parent) {
        ...
        switch (type) {
            ...
            case ComplexTypes.Action:
                return views.Action.fromJSON(parent, jsonData);
            ...
            case ComplexTypes.TableItem:
                return views.TableItem.fromJSON(parent, jsonData);
            default:
