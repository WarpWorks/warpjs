...
function createResourceFromDocument(instance) {
    ...
    return createResource(RoutesInfo.expand('hs-app:app', data), instance);
}

function basicRender(name, data, req, res) {
    const resource = (data instanceof hal.Resource) ? data : createResource(req, data);
    resource.baseUrl = req.app.get('hs-app:baseUrl');

    resource.link('hsAppDomain', RoutesInfo.expand('hs-app:app', data));
